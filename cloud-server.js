...
const pool = process.env.DATABASE_URL ? new Pool({connectionString:process.env.DATABASE_URL,ssl:{rejectUnauthorized:false},max:20}) : new Pool({host:process.env.DB_HOST||'127.0.0.1',port:Number(process.env.DB_PORT||5432),user:process.env.DB_USER||'friendssoft',password:process.env.DB_PASSWORD||'',database:process.env.DB_NAME||'friendssoft',max:20,ssl: process.env.DB_SSL === 'false' ? false : {rejectUnauthorized:false}});
function pgSql(sql){ let i=0; return sql.replace(/\?/g,()=>'$'+(++i)); }
async function query(sql, params=[]){ return pool.query(pgSql(sql), params); }
